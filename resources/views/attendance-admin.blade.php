...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        ...
                        <form method="POST"
                              action="{{ route('schedule.attendance.update', $schedule->id) }}">
                            @csrf
                                    ...
                                    <tbody>
                                        @foreach ($schedule->participants as $index => $participant)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>
                                                    {{ $participant->learner->first_name }}
                                                    {{ $participant->learner->last_name }}
                                                </td>
                                                <td class="text-center">
                                                    <span
                                                        class="badge attendance-toggle
                                                        {{ $participant->status === 'PRESENT' ? 'badge-success' : 'badge-danger' }}"
                                                    ...
                                                    <input
                                                        ...
                                                        id="attendance-input-{{ $participant->id }}"
                                                    >
                                                </td>
    ...
    <!-- Page level custom scripts -->
    <script>
    document.querySelectorAll('.attendance-toggle').forEach(badge => {
    badge.addEventListener('click', function () {
        ...
        if (status === 'PRESENT') {
            ...
            document.getElementById('attendance-input-' + participantId).value = 'ABSENT';
        } else {
            // ubah ke PRESENT
            this.dataset.status = 'PRESENT';
            this.innerText = 'PRESENT';
            this.classList.remove('badge-danger');
            this.classList.add('badge-success');
            document.getElementById('attendance-input-' + participantId).value = 'PRESENT';
        }
    });
});
</script>
