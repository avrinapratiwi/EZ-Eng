...
<body id="page-top">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3 d-flex justify-content-between align-items-center">
                            <h6 class="m-0 font-weight-bold text-primary">Schedule Data</h6>
                            <a href="#" class="btn btn-success btn-icon-split mb-3" data-toggle="modal" data-target="#addScheduleModal">
                                        ...
                                        @forelse ($schedules as $index => $schedule)
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    <!-- ATTENDANCE -->
                                                    <a href="{{ route('schedule.attendance', $schedule->id) }}"
                                                        class="btn btn-warning btn-icon-split btn-sm mb-1">
                                                         <span class="icon text-white-50">
                ...
                <form action="{{ route('schedules.store') }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="mb-2">
                                <label>
                                    <input type="checkbox" id="selectAllLearners">
                                    Select All Learners
                                </label>
                            </div>
                                ...
                                @foreach ($learners as $learner)
                                    <div class="form-check">
                                        <input
                                            ...
                                            name="learners[]"
                                            value="{{ $learner->id }}"
                                            class="form-check-input learner-checkbox"
                                            id="learner{{ $learner->id }}"
                                        >
                                        <label class="form-check-label" for="learner{{ $learner->id }}">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="url" name="meeting_link" class="form-control"
                                value="{{ $schedule->meeting_link }}" required>
                        </div>
                            ...
                            <select name="mentor_id" class="form-control" required>
                                @foreach ($mentors as $mentor)
                                    <option value="{{ $mentor->id }}"
                                        {{ $schedule->mentor_id == $mentor->id ? 'selected' : '' }}>
                                        {{ $mentor->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="meeting_date" class="form-control"
                                value="{{ $schedule->meeting_date }}" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="time" name="meeting_time" class="form-control"
                                value="{{ $schedule->meeting_time }}" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="learner-box" style="max-height:200px; overflow-y:auto; border:1px solid #ddd; padding:10px;">
                                @foreach ($learners as $learner)
                                    <div class="form-check">
                                        <input type="checkbox"
                                            name="learners[]"
                                            ...
                                            id="edit{{ $schedule->id }}_{{ $learner->id }}"
                                            {{ $schedule->participants->contains('learner_id', $learner->id) ? 'checked' : '' }}>
                                        <label class="form-check-label"
                                            for="edit{{ $schedule->id }}_{{ $learner->id }}">
                                            {{ $learner->first_name }} {{ $learner->last_name }}
                                        </label>
                                    </div>
                                @endforeach
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <form method="POST" action="{{ route('schedules.delete', $schedule->id) }}">
                        @csrf
                        @method('DELETE')
    ...
    <script>
        ...
        document.getElementById('selectAllLearners').addEventListener('change', function () {
            ...
            document.querySelectorAll('.learner-checkbox').forEach(cb => {
                cb.checked = checked;
            });
        });
    
        // Jika salah satu learner di-uncheck → select all ikut mati
        document.querySelectorAll('.learner-checkbox').forEach(cb => {
            cb.addEventListener('change', function () {
                const allChecked = document.querySelectorAll('.learner-checkbox:checked').length
                                 === document.querySelectorAll('.learner-checkbox').length;
    
                document.getElementById('selectAllLearners').checked = allChecked;
            });
        });
    </script>
